...
class TripMapController extends Controller
{
    ...
    public function __construct(){
        ...
        $ordering = TripMapModel::max('ordering');
    ...
    public function index()
    {
        ...
        $data = TripMapModel::where('trip_id',$trip_id)->get();
        return view('admin.trip-map.index',compact('data','trip_id'));
    }
    ...
    public function create()
    {
        ...
        return view('admin.trip-map.create',compact('trip_id','ordering'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if($request->hasfile('thumbnail')){
            ...
            $file_name = Str::slug( 'map-'.$category_file[0]) . '-' . Str::random(20) . '.' . $extension;
    ...
    {
        $data = TripMapModel::find($id);
        $trip_id = $this->trip_id;
        $ordering = $data->ordering;
        return view('admin.trip-map.edit',compact('data','trip_id','ordering'));
    }
    ...
    public function update(Request $request, $trip_id, $id)
    {
        ...
        if($request->hasfile('thumbnail')){
            ...
            $extension = $request->file('thumbnail')->getClientOriginalExtension();
            $category_file = explode('.', $category_file);
            $file_name = Str::slug($category_file[0]) . '-' . Str::random(20) . '.' . $extension;
    ...
    public function maps_thumb_update(Request $request, $id){
        // $request->validate([
        //     'thumbnail' => 'required|mimes:png,jpg,jpeg|max:2048',
        // ]);
        $data = TripMapModel::find($id);
