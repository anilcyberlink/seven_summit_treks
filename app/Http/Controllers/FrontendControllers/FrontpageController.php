...
class FrontpageController extends Controller
{
    public function index()
    {
        ...
        $articles = PostModel::where(['post_type'=>'8','status'=>'1'])->orderBy('post_order', 'asc')->take(3)->get();
        $settings = SettingModel::all();
        
        return view('themes.default.frontpage', compact('banners', 'reviews', 'regions', 'blogs', 'blog_thumbs', 'expeditions', 'settings','articles'));
    }

    public function pagedetail($uri)
    {
                $articles = PostModel::where(['post_type'=>'8','status'=>'1'])->orderBy('id', 'desc')->take(3)->get();
        if (!check_uri($uri)) {
            abort(404);
        }
        ...
        $data_child = PostModel::where('post_parent', $data['id'])->where('status','1')->orderBy('post_order', 'asc')->paginate(20);
        ...
        $articles = PostModel::where(['post_type'=>'8','status'=>'1'])->orderBy('id', 'desc')->paginate(12);
        $latest_article_post = PostModel::where(['post_type'=>'5','status'=>'1'])->orderBy('id', 'desc')->first();
        ...
        'downloads', 'faq', 'activities', 'trips', 'country', 'team', 'team_cat', 'first_team', 'message', 'mountains', 'achievements', 'certificates','postimage','trek','exp','articles','latest_article_post'));
    }
    ...
    public function tripdetail($uri)
    {
        ...
        //by sangam stars
        $trip_maps = $data->maps;
        // If $data->trip_map is not empty, merge it with the $trip_maps collection
        if ($data->trip_map) {
            // Create a new collection with the trip_map image
            $trip_map_collection = collect([(object) ['thumbnail' => $data->trip_map,'title'=>$data->trip_title]]);

            // Merge the trip_map collection with the $trip_maps collection
            $trip_maps = $trip_map_collection->merge($trip_maps);
        }
        //by sangam ends
        ...
        return view('themes.default.tripdetail', compact('data', 'rating', 'trip_docs', 'routecamps', 'trip_review',
            ...
            'tripinsurance2','application1','application2','photos','videos','trip_maps'));
    }
    ...
    public function expeditionlist($uri)
    {
        ...
        $blogLength = Str::length($data->content);
        return view('themes.default.' . $template, compact('data', 'trips', 'facts', 'expeditionGroup', 'expeditions','blogLength'));
    }
...
//test pass//
    ...
    {
        // dd($parenturi,$uri);
        $parent = PageTypeModel::where(['uri' => $parenturi])->first();
        ...
        $related = PageModel::where(['page_type' => $parent->id, 'status' => '1'])->where('uri', '!=', $uri)->orderBy('page_order', 'asc')->get();

        $links = PageTypeModel::where(['is_menu' => '1'])->orderBy('ordering', 'asc')->get();

        return view('themes.default.usefulinfo-detail', compact('data', 'details', 'links','doc','related','parent'));
    }
...
//        }
  ...
  public function trip_cost_inc(Request $request)
    {       
      $cost_includes=TestimonialModel::where(["category" => $request->trip_category, 'trip_id' => $request->trip_id])->orderBy('ordering','asc')->get();
      $cost_excludes=TripInfoModel::where(["category" => $request->trip_category, 'trip_id' => $request->trip_id])->orderBy('ordering','asc')->get();
    ...
    public function medical_assessment_success(Request $request)
    {
        ...
        try {
            $medicalApplication = MedicalAssessment::create([
                ...
                'doctor_comment'  => $validatedData['doctor_comment'],
