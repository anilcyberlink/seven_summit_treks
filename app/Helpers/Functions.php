...
function getCanonicalUrl()
{
 ...
 $url = request()->url();
 // Optionally enforce HTTPS, remove www, etc.
 $url = str_replace('http://', 'https://', $url); // Force HTTPS
 $url = str_replace('www.', '', $url); // Remove www if not preferred
 return rtrim($url, '/'); // Remove trailing slash if desired
}

function getPostTypeIdByUri($uri)
{
    $data = PostTypeModel::where('uri', $uri)->first();
    if ($data) {
        return $data->id;
