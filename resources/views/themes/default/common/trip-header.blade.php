...
<head>
   ...
   <meta name="title" content="@yield('meta_keyword')">
    ...
    @else
    <meta property="og:image" content="{{asset(env('PUBLIC_PATH').'/images')}}/logo.svg"/>
    @endif
...
<nav class="uk-navbar d-flex uk-flex-middle" uk-navbar>
    <div class="uk-navbar-left ">
        ...
        <img src="{{ asset('themes-assets/images/tm-white-logo.png') }}" alt="{{ $setting->site_name }}" class="uk-logo-white">
            <img src="{{ asset('themes-assets/images/new-logo.svg') }}" alt="{{ $setting->site_name }}" class="uk-logo-dark">
            <img src="{{ asset('themes-assets/images/tm-gold-logo.png') }}" alt="{{ $setting->site_name }}" class="uk-logo-primary"> </a>
    </div>
                                    ...
                                    <div class="mega-border-top">
                                        <div class="uk-padding-menu">
                                            <ul class="uk-child-width-1-4 uk-flex" uk-grid uk-scrollspy="cls: uk-animation-slide-top-small; delay: 300; repeat: false;">
                                                <!-- -->
                                                @if (has_posts($row->id))
                                                    @foreach(has_posts($row->id)->chunk(4) as $row)
                                                        <li>
                                                            <div>
                                                                <ul class="uk-menu-list">
                                                                    @foreach ($row as $_row)
                                                                        ...
                                                                        @else
                                                                            @if($_row->id == 168)
                                                                                <li>
                                                                                    <a href="{{ url(geturl($_row->uri)) }}" style="color:#d62a34!important;">{{ Str::ucfirst($_row->post_title) }}</a>
                                                                                </li>
                                                                            @else
                                                                                <li>
                                                                                    <a href="{{ url(geturl($_row->uri)) }}">{{ Str::ucfirst($_row->post_title) }}</a>
                                                                                </li>
                                                                            @endif
                                                                        @endif
                                                                    @endforeach
                                                        ...
                                                        <div>
                                                            <ul class="uk-menu-list">
                                                                @foreach (has_posts($row->id) as $_row)
                                                                    @if($_row->uri == 'our-team')
                                                                        <li>
                                                                            <a href="{{ url(geturl($_row->uri)) }}" class="uk-active">{{ Str::ucfirst($_row->post_title) }}</a>
                                                                        </li>
                                                                    @else
                                                                        @if($_row->id == 168)
                                                                            <li>
                                                ...
                                                <!--        <a href="{{route('all-trip')}}" class="uk-button uk-button-white-outline uk-width-1-1">-->
...
<!-- start mobile menu -->
  <div id="offcanvas-reveal" class="uk-modal-full" uk-modal>
    ...
    <div>
        <a class="uk-navbar-item uk-logo" href="{{ url('/') }}"> 
        <img src="{{ asset('/themes-assets/images/tm-gold-logo.png') }}" alt="{{ $setting->site_name }}" class="uk-logo-light" width="110"> </a>
    </div>
    <div>
  ...
  <nav class="uk-margin-large-top uk-margin-large-bottom">
    ...
    @if ($company->count())
        <li class="uk-parent"> <a href="javascript:void(0)">About Us </a>
            <ul class="uknavsub">
                @foreach ($footer2 as $row)
                    @if($row->uri == 'our-team')
                        <li> <a href="{{ url(geturl($row->uri)) }}" class="uk-active">{{ Str::ucfirst($row->post_title) }}</a> </li>
                    @else
                        @if($row->id == 168)
                            <li>
                                <a href="{{ url(geturl($row->uri)) }}" style="color:#d62a34!important;">{{ Str::ucfirst($row->post_title) }}</a>
                            </li>
                        @else
                            <li>
                                <a href="{{ url(geturl($row->uri)) }}">{{ Str::ucfirst($row->post_title) }}</a>
                            </li>
                        @endif
                    @endif
                @endforeach
