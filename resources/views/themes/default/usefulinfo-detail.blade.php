				...
				<div class="uk-width-1-4@m uk-visible@m">
					...
					<div class="uk-blog-sidebar " style="z-index: 9;" uk-sticky="offset: 100; bottom: #uk-stop-sticky">
						...
						<div>
							...
							<a href="{{ $row->external_link }}" class="{{$row->uri ==  Request::segment(2)?'uk-active':''}}"> {{$row->page_title}} </a> 
							@else
							<a href="{{ url('info/'.$parent->uri.'/'.teamurl($row['uri'],$row['page_key'])) }}" class="{{$row->uri ==  Request::segment(2)?'uk-active':''}}"> {{$row->page_title}} </a> 
							@endif
							</li>
								@endforeach
							</ul>
