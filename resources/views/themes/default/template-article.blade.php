        ...
        <div class="uk-container uk-position-relative">
            ...
            <div class="uk-grid-medium" uk-grid
                uk-scrollspy="cls: uk-animation-slide-left-small; target:.uk-article;  delay: 300; repeat: false;">
                 @if ($latest_article_post)
                    <div class="uk-position-relative uk-width-1-1">
                        <a class="uk-article uk-inline-clip uk-link-toggle uk-width-1-1" href="{{ url(geturl($latest_article_post->uri)) }}">
                            <div class="uk-overlay-primary  uk-position-cover"></div>
                            <div class="uk-media-500"> 
                            @if($latest_article_post->page_thumbnail)
                            <img class="uk-image" uk-img="" alt="{{ $latest_article_post->post_title }}"
                                    src="{{ asset('uploads/original/' . $latest_article_post->page_thumbnail) }}"> 
                            @else
                              <img class="uk-image" uk-img="" alt="{{ $latest_article_post->post_title }}"
                                ...
                                <div class="uk-text-left uk-position-z-index">
                                    <div class="uk-width-1-2@m text-white">
                                        <div class="text-white uk-h5 uk-margin-top uk-margin-remove-bottom">
                                            {{ dateformat($latest_article_post->post_date) }}
                                        </div>
                                        <h2
                                            class="text-white  theme-font-medium-bold uk-margin-small-top uk-margin-small-bottom ">
                                            {{ $latest_article_post->post_title }}
                                        </h2>
                                        {!! $latest_article_post->post_excerpt !!}
                                    </div>
                                </div>
                            </div>
                        </a>
                ...
                <!--  -->
				  ...
				  <div class=" uk-child-width-1-2@s uk-child-width-1-3@m  uk-grid-medium uk-grid" uk-grid=""
                    ...
                    <!--  -->
                    @if ($articles->count() > 0)
                        @foreach ($articles->slice(1,12) as $row)
                            <div>
                ...
                <!--  -->
                {{ $articles->links('themes.default.common.paginate') }}
