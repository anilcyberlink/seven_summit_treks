...
@section('breadcrumb')
    ...
    <a href="{{ route('category.index', $data->id) }}" class="btn btn-default btn-sm backlink"><i class="fa fa-list"
                        ...
                        <div class="card">
                            <div class="card-header d-flex p-0">
                                <ul class="nav nav-pills ml-auto p4 mb10 mt10 nav-custom">
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link" href="#tab_6" data-toggle="tab"> MAPS/ROUTES
                                        </a>
                                    </li>
                            ...
                            <div class="card-body">
                                <div class="tab-content">
                                    ...
                                    <div class="tab-pane" id="tab_6">
                                        @include('admin.trips.edit.edit-map')
                                    </div>
                                    ...
                                    <div class="tab-pane" id="tab_8">
                                        @include('admin.trips.edit.edit-route-camps')
                                    </div>

                                    <div class="tab-pane" id="tab_9">
                                        @include('admin.trips.edit.edit-weather-reports')
                                    </div>
                                     <div class="tab-pane" id="tab_10">
                                      @include('admin.trips.edit.edit-highlights')                      
                                      </div>
                                    <!-- /.tab-pane -->
    ...
    jQuery(document).delegate('button.delete-highlights', 'click', function(e) {
     ...
     if (makeConfirm == true) {
      ...
      // For delete highlights individually.              
        ...
        var team_id = '{{$data->id}}';
        var url = '{{ route("highlights.destroy",["id"=>':id',"info_id"=>':info_id']) }}';
...
/******** End For highlights *******/
        ...
        /******** End For Gear *******/
        /******** For Map *******/
        jQuery(document).delegate('a.add-map', 'click', function(e) {
            e.preventDefault();
            var content = jQuery('#row_map_additional .row'),
                size = jQuery('#row_map_body >.row').length + 1,
                element = null,
                element = content.clone();
            element.attr('id', 'map-rec-' + size);
            element.find('.delete-map').attr('map-data-id', size);
            element.appendTo('#row_map_body');
            element.find('.sn').html(size);
        });

        jQuery(document).delegate('button.delete-map', 'click', function(e) {
            e.preventDefault();
            var makeConfirm = confirm("Are you sure You want to delete");
            if (makeConfirm == true) {
                var id = jQuery(this).attr('map-data-id');
                var targetDiv = jQuery(this).attr('targetDiv');
                // For delete sehedule individually.
                var csrf = $('meta[name="csrf-token"]').attr('content');
                var map_rowid = jQuery(this).attr('map-rowid');
                var trip_id = '{{ $data->id }}';
                var url =
                    '{{ route('admin.trip-map.destroy', ['admin' => ':admin', 'trip_map' => ':trip_map']) }}';
                url = url.replace(':admin', trip_id);
                url = url.replace(':trip_map', map_rowid);
                if (map_rowid) {
                    $.ajax({
                        type: 'DELETE',
                        url: url,
        ...
        jQuery(document).delegate('button.delete-routecamp', 'click', function(e) {
            ...
            if (makeConfirm == true) {
                ...
                var trip_id = '{{ $data->id }}';
                var url = '{{ route('routecamp.destroy', ['id' => ':id', 'routecamp_id' => ':routecamp_id']) }}';
                url = url.replace(':id', trip_id);
        ...
        // Delete map thumb
        jQuery(document).delegate('.delete_maps_thumb', 'click', function(e) {
            e.preventDefault();
            if (!confirm('Are you sure to delete?')) return false;
            var csrf = $('meta[name="csrf-token"]').attr('content');
            var id = $(this).attr('href');
            var url = '{{ route('delete_maps_thumb', ['id' => ':id']) }}';
            url = url.replace(':id', id);
            $.ajax({
                type: 'DELETE',
                url: url,
  ...
  /******/
   $('.bannerdelete').on('click',function(e){
    ...
    $.ajax({
      type:'delete',
      url:"{{url('delete_trip_banner_thumb') . '/'}}" + id,
      data:{_token:csrf},    
      success:function(data){ 
  ...
  //mapdelete
    ...
    $.ajax({
      type:'delete',
      url:"{{url('delete_map_thumb') . '/'}}" + id,
      data:{_token:csrf},    
      success:function(data){ 
        $('div.map_id' + id ).remove();
        if (data.status == 'success') {
            toastr.success(data.message);
        }
      },
