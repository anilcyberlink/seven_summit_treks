                        ...
                        <table class="table admin-form table-striped dataTable" id="datatable3">
                            <thead>
                                <tr class="bg-light">
                                    ...
                                    <th>Post Name</th>
                                    <th>Status</th>
                                    @if(getPostTypeIdByUri(Request::segment(2)) == 1 && Request::segment(3) == null)
                                        <th>Is Footer?</th>
                                    @endif
                                    <th>Order</th>
                                    <th></th>
                                        ...
                                        @if(getPostTypeIdByUri(Request::segment(2)) == 1 && Request::segment(3) == null)
                                            <td>
                                                @if($row->post_parent == 0)
                                                    <input class="CheckFooter" type="checkbox" name="is_footer" data-rowid="{{$row->id}}" {{($row->is_footer == 1)?'checked':''}} />
                                                @else
                                                    -
                                                @endif
                                            </td>
...
@section('libraries')
    ...
    <script type="text/javascript">
        ...
        $('.CheckFooter').on('click',function(e){
            var csrf = $('meta[name="csrf-token"]').attr('content');
            var id = $(this).attr('data-rowid');
            var url  = '{{ route("admin.post.footer",["id"=>':id']) }}';
            url = url.replace(':id',id);
            $.ajax({
                type:'put',
                url:url,
